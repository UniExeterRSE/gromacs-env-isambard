...
# Default selectors so a bare `module load` picks a sensible target: cray-neon is
# the project default variant (system MPI/FFT, and the SIMD that measured
# fastest -- see MAINTAINER.md "Tuning"), and the most-recently-built version becomes the
...
-- Generated by gen-modulefile.sh. Within $GROMACS_ENV_VERSION, default variant = cray-neon.
module_version("gromacs-env/$GROMACS_ENV_VERSION/cray-neon", "default")
EOF
cat > "$MODULEFILES_DIR/gromacs-env/.modulerc.lua" <<EOF
-- Generated by gen-modulefile.sh. Bare 'module load gromacs-env' default =
-- the most recently built version's cray-neon variant.
module_version("gromacs-env/$GROMACS_ENV_VERSION/cray-neon", "default")
EOF
